/*
* This header is generated by classdump-dyld 1.0
* on Tuesday, June 5, 2018 at 10:42:46 PM China Standard Time
* Operating System: Version 8.2 (Build 12D508)
* Image Source: /private/var/db/stash/_.8k0vmU/Applications/Stocks.app/Stocks
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Stocks/Stocks-Structs.h>
#import <UIKit/UIView.h>
#import <UIKit/UITableViewDelegate.h>
#import <UIKit/UITableViewDataSource.h>
#import <Stocks/StockUpdateObserver.h>

@class UITableView, Stock, NSMutableArray, NSString;

@interface StocksListView : UIView <UITableViewDelegate, UITableViewDataSource, StockUpdateObserver> {

	UITableView* _stocksTable;
	Stock* _selectedStockForSnapshot;
	Stock* _selectedStock;
	Stock* _lastSelectedStock;
	BOOL _syncInProgress;
	CGPoint _contentOffset;
	CGPoint _selectedStockOrigin;
	NSMutableArray* _visibleStocks;
	CGPoint _visibleStocksOrigin;
	CGPoint _listTapDownLocation;
	double _scrollPointToRestore;
	long long _rowDataType;
	UIEdgeInsets _scrollInsets;

}

@property (nonatomic,retain) Stock * selectedStock;                  //@synthesize selectedStock=_selectedStock - In the implementation block
@property (assign,nonatomic) long long rowDataType;                  //@synthesize rowDataType=_rowDataType - In the implementation block
@property (assign,nonatomic) UIEdgeInsets scrollInsets;              //@synthesize scrollInsets=_scrollInsets - In the implementation block
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(void)setSelectedStock:(Stock *)arg1 ;
-(void)scrollToSelectedStockAnimated:(BOOL)arg1 ;
-(Stock *)selectedStock;
-(void)setRowDataType:(long long)arg1 ;
-(void)loadRowDataType;
-(void)_stocksWillSync;
-(void)_stocksDidChange;
-(void)_stockShortNameDidChange:(id)arg1 ;
-(void)listTapped:(id)arg1 ;
-(id)_visbleStocksWithStock:(id)arg1 atVisiblePosition:(unsigned long long)arg2 ;
-(double)requiredRowCount;
-(void)restoreScrollPoint:(double)arg1 ;
-(void)setScrollInsets:(UIEdgeInsets)arg1 ;
-(id)lastSelectedStock;
-(long long)rowDataType;
-(UIEdgeInsets)scrollInsets;
-(id)initWithFrame:(CGRect)arg1 ;
-(void)dealloc;
-(void)layoutSubviews;
-(id)tableView:(id)arg1 willSelectRowAtIndexPath:(id)arg2 ;
-(void)scrollViewDidEndDragging:(id)arg1 willDecelerate:(BOOL)arg2 ;
-(void)scrollViewDidEndDecelerating:(id)arg1 ;
-(void)scrollViewDidEndScrollingAnimation:(id)arg1 ;
-(long long)tableView:(id)arg1 numberOfRowsInSection:(long long)arg2 ;
-(id)tableView:(id)arg1 cellForRowAtIndexPath:(id)arg2 ;
-(CGSize)intrinsicContentSize;
-(id)tableView;
-(void)stockUpdateManager:(id)arg1 didUpdateStocks:(id)arg2 updates:(long long)arg3 ;
@end

